    ...
    public function loadRepositories(array $config, ContainerBuilder $container)
    {
        ...
        foreach ($repositories as $name => $repositoryConfig) {
            if (isset($repositoryConfig['entity'])) {
                ...
                $operatorDefinition = new Definition(
                    '%webdev_media.operator.class%',
                    array($repositoryConfig['property'], $repositoryConfig['path'],));
                $operatorDefinition->addMethodCall('setRootPath', array('%webdev_media.root_path%'));
                // Add the cache path if its set
                if(isset($repositoryConfig['derivatives'])) {
                    $operatorDefinition->addMethodCall('addDerivativePath',array($repositoryConfig['derivatives']));
                }
                ...
                $definition = $container->setDefinition($definitionID, new DefinitionDecorator(
                                        "media.doctrine_repository_template"));
                $definition->setArguments(array(new Reference(
                                                sprintf("doctrine.orm.%s_entity_manager", $repositoryConfig['entity_manager'])),
                ...
                $container->getDefinition('media.manager')
                          ->addMethodCall('addRepository', array(new Reference($definitionID)));

                // Create the entity media injector listener
                $injectorDefinition = new Definition(
                    '%webdev_media.entity_repository.injector.class%',
                    array($repositoryConfig['entity'], new Reference($operatorDefinitionID),));
